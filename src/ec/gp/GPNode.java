...
public abstract class GPNode implements GPNodeParent, Prototype {
    ...
    public int atDepth() {
        ...
        while (cparent instanceof GPNode) {
            count++;
    ...
    public String errorInfo() {
        return "GPNode " + this + " in the function set for tree " + ((GPTree) (rootParent())).treeNumber();
    }
    ...
    protected String makeGraphvizSubtree(String prefix) {
        StringBuilder body = new StringBuilder(prefix + "[label = \"" + toStringForHumans() + "\"];\n");
        for (int x = 0; x < children.length; x++) {
            String newprefix;
            if (x < 10) newprefix = prefix + x;
            else newprefix = prefix + "n" + x;  // to distinguish it

            body.append(children[x].makeGraphvizSubtree(newprefix));
            body.append(prefix).append(" -> ").append(newprefix).append(";\n");
        }
    ...
    public String makeLatexTree() {
        ...
        StringBuilder s = new StringBuilder("\\begin{bundle}{\\gpbox{" + toStringForHumans() + "}}");
        for (int x = 0; x < children.length; x++)
            s.append("\\chunk{").append(children[x].makeLatexTree()).append("}");
        s.append("\\end{bundle}");
        return s.toString();
    }
    ...
    public String makeCTree(boolean parentMadeParens, boolean printTerminalsAsVariables, boolean useOperatorForm) {
        if (children.length == 0)
            ...
            StringBuilder s = new StringBuilder(toStringForHumans() + "(" + children[0].makeCTree(true, printTerminalsAsVariables, useOperatorForm));
            for (int x = 1; x < children.length; x++)
                s.append(", ").append(children[x].makeCTree(true, printTerminalsAsVariables, useOperatorForm));
            return s + ")";
        }
    }
