        ...
        Runnable run = new Runnable() {
            ...
            void initializeEvolutionState()
                ...
                */
                ...
                // one per thread
                MersenneTwisterFast[] random = new MersenneTwisterFast[Math.max(breedthreads, evalthreads)];
                int[] seeds = new int[Math.max(breedthreads, evalthreads)];

                StringBuilder seed_message = new StringBuilder("Seed: ");
                for (int x = 0; x < random.length; x++) {
                    seeds[x] = conPanel.getSeed(currentJob, x);
                    seed_message.append(seeds[x]).append(" ");
                }
                ...
                output.systemMessage(seed_message.toString());
