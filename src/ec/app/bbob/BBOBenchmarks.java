...
public class BBOBenchmarks extends Problem implements SimpleProblemForm {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        switch (problemType) {
            ...
            default:
                StringBuilder outputStr = new StringBuilder("Invalid value for parameter, or parameter not found.\n" + "Acceptable values are:\n");
                for (i = 0; i < problemTypes.length; i++)
                    outputStr.append(problemTypes[i]).append("\n");
                state.output.fatal(outputStr.toString(), base.push(P_WHICH_PROBLEM));
        }
            ...
            case SPHERE:// f1
                ...
                switch (noise) {
                    ...
                    default:
                        StringBuilder outputStr = new StringBuilder("Invalid value for parameter, or parameter not found.\n" + "Acceptable values are:\n");
                        for (i = 0; i < noiseTypes.length; i++)
                            outputStr.append(noiseTypes[i]).append("\n");
                        state.output.fatal(outputStr.toString(), new Parameter(P_NOISE));
                        break;
                }
                value += fAdd;
                fit = (-value);
