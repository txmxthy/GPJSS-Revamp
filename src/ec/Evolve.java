...
public class Evolve {
    ...
    public static EvolutionState initialize(ParameterDatabase parameters, int randomSeedOffset, Output output) {
        ...
        random = new MersenneTwisterFast[Math.max(breedthreads, evalthreads)];
        seeds = new int[random.length];

        StringBuilder seedMessage = new StringBuilder("Seed: ");
        ...
        for (x = 0; x < random.length; x++) {
            ...
            seedMessage.append(seeds[x]).append(" ");
        }
        ...
        output.systemMessage(seedMessage.toString());
