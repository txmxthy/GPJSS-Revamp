package solvers.algorithm.elitesbasedpop;
...
import simulation.jss.helper.PopulationUtils;
import simulation.jss.surrogate.Surrogate;
import simulation.rules.ruleoptimisation.RuleOptimizationProblem;
import solvers.algorithm.adaptivepop.multiPopCoevolutionaryClearingEvaluator;
    ...
    public int evolve() {
        ...
        if (generation == preGenerations) {
            ...
            for (int i = 0; i < population.subpops.length; i++) { //before here is ++i
                ...
                double fitUB = selIndis.get(0).fitness.fitness();  //the upper bound of fitness   //the best one, the smallest fitness in the individuals  1475.0335218025866
                double fitLB = 1 - fitUB;  //the lower bound of fitness  -1474.0335218025866
            ...
            for (int rep = 0; rep < times; rep++) {
                NSGA2Population();//fzhang 2019.6.16 improve the quality of adaptPopulation
            }
...
//            //for niching
    ...
    /**
     * fzhang 2019.6.16
     * after adaptPopulation, to improve the quality of population in stage 2
     * 1. breed a new population based on adaptPopulation
     * 2. choose 512 best individuals for each subpop among the parent population and offspring population---similar to NSGA-II
     */
    public void NSGA2Population() {
        ...
        PopulationUtils.sort(newPop);
        for (int subpop = 0; subpop < population.subpops.length; subpop++) {
            System.arraycopy(newPop.subpops[subpop].individuals, 0, population.subpops[subpop].individuals, 0, population.subpops[subpop].individuals.length);
        }
    }
}
