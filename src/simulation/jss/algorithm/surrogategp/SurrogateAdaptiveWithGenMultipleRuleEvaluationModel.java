package simulation.jss.algorithm.surrogategp;
...
public class SurrogateAdaptiveWithGenMultipleRuleEvaluationModel extends MultipleRuleEvaluationModel implements Surrogate {
			...
			/*if (state.generation < 15) {
 			...
 			//int numMachine = 2;
 				...
 				if(state.generation == 0) {
 					surrogateSet = schedulingSet.surrogate(10, 100, 20, objectives);
 					//System.out.println("generation 0");
 				}
 				else {
					//int numJobs = (int) Math.pow(5000*state.generation/(state.numGenerations-1), 1+state.generation/(state.numGenerations-1));
				    surrogateSet = schedulingSet.surrogate(10, 5000*state.generation/(state.numGenerations-1), 1000*state.generation/(state.numGenerations-1), objectives);
