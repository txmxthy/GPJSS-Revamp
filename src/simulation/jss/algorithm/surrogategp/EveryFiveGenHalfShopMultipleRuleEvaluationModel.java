...
public class EveryFiveGenHalfShopMultipleRuleEvaluationModel extends MultipleRuleEvaluationModel implements Surrogate {
 ...
 // ========================rule evaluatation============================
 	...
 	/*	if(useSurrogate)
 		...
 		//2---fzhang 6.7.2018  use different surrogates in different several generations
		if (useSurrogate) {
			
			// fzhang 6.7.2018 use different surrogates in different several generations
			if (state.generation % 3 == 0) {
				surrogateSet = schedulingSet.surrogate(10, 5000, 1000, objectives);
				//System.out.print("I am the five generation points");
			}
				
			else {
				surrogateSet = schedulingSet.surrogate(5, 2500, 500, objectives);
				//System.out.println("not the five generation points");
			}
