package simulation.jss.algorithm.adaptivepop;
...
public class fracCrossoverPipeline extends CrossoverPipeline {
...

    {
        ...
        for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
        {
            ...
            if (sources[0]==sources[1])  // grab from the same source  always true, even tc0 and tc1 appear
                sources[0].produceFrac(2,2,0,subpopulation,parents,state,thread);  //max start subpopulation
