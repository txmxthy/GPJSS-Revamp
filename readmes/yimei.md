...
3. The ECJ functions are located at `src/ec/`, and the functions for JSS and GP are in `src/simulation/jss/`. Now you are ready to run a number of different algorithms.
...
The main project is located in `/src/simulation/jss/`. It contains the following packages:
...
1. Locate the param file `src/simulation/jss/algorithm/simplegp/simplegp.params`
2. Run `src/simulation/jss/gp/GPRun` with argument "*-file [pathofparamsfile]/simplegp.params*". You can also set other parameters using "*-p xxx*", in the same way as any ECJ applications.
...
Yi Mei, Mengjie Zhang, Su Nguyen, "<a href="http://homepages.ecs.vuw.ac.nz/~simulation/papers/GECCO16-MeiSuZhang.pdf">Feature Selection in Evolving Job Shop Dispatching Rules with Genetic Programming</a>," *Genetic and Evolutionary Computation Conference (GECCO)*, Denver, USA, 2016.
...
2. Run `src/simulation/ruleanalysis/RuleTestFeatureContribution` using the argument "*dir/ simple-rule 30 dynamic-job-shop missing-0.85-4 1 max-tardiness relative-terminals*".
...
1. Run the params file `src/simulation/algorithms/surrogategp/surrogategp.params`
